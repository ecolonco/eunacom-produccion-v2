...
async function checkConfidenceScores() {
  ...
  const activeWithQA = await prisma.$queryRaw<Array<{
    variation_id: string;
    display_code: string;
    version: number;
  ...
  console.log(`\n✅ Variaciones activas con QA: ${activeWithQA.length > 0 ? 'encontradas' : 'no encontradas'}`);

  if (activeWithQA.length > 0) {
    console.log('\n📋 Primeras 20 variaciones con scores más bajos:');
    console.log('ID\t\tDisplay Code\tVersion\tConfidence\tFecha QA');
    console.log('─'.repeat(80));

    activeWithQA.forEach(v => {
      const score = (v.confidence_score * 100).toFixed(1);
      const date = new Date(v.created_at).toLocaleDateString();
      console.log(`${v.variation_id.slice(0, 8)}...\t${v.display_code}\tv${v.version}\t${score}%\t\t${date}`);
    });
  }
  ...
  const neverAnalyzed = await prisma.$queryRaw<Array<{ count: string }>>`
  ...
  if (neverCount > totalActive * 0.5) {
    console.log('   - Más del 50% nunca fue analizado. Ejecuta QA Sweep en todos los ejercicios.');
  }
