 ...
 */
function calculateConfidenceScore(evaluation: any): number {
 ...
 */
function calculateImprovedConfidence(severityGlobal: number): number {
...
async function backfillConfidenceScores() {
  ...
  try {
    ...
    const variationsWithQA = await prisma.$queryRaw<Array<{
      variation_id: string;
    ...
    for (const corrected of correctedVariations) {
      ...
      // Primero intentamos con parentVersionId, si no existe, buscamos la v1 de la misma base
      ...
      if (qaResult && qaResult.diagnosis) {
        const diagnosis = qaResult.diagnosis as any;
