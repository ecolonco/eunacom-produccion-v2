...
export class QASweep2Service {
  ...
  private async getVariationsForAnalysis(
    ...
    try {
      ...
      if (maxConfidenceScore !== undefined && maxConfidenceScore !== null) {
        ...
        logger.info('Filtering by max confidence score (excluding perfect 100%)', {
          maxConfidenceScore,
          scoreThreshold,
          actualThreshold
        });
  ...
  /**
   * Obtiene los resultados de un run con displayCode de variaciones
   */
  async getRunResults(runId: string): Promise<any> {
    try {
      ...
      // Agregar displayCode a cada resultado
      if (run && run.results) {
        run.results = run.results.map((result: any) => ({
          ...result,
          displayCode: result.variation?.displayCode || result.variationId
        }));
      }
