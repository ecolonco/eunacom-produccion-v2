...
router.delete('/all-exercises', authenticate, async (req, res) => {
  try {
...
router.delete('/taxonomy-safe', authenticate, async (req, res) => {
  try {
    // Only allow admin users
    if (req.user?.role !== 'ADMIN') {
      return res.status(403).json({
...
router.post('/reset-sequences', authenticate, async (req, res) => {
  try {
    // Only allow admin users
    if (req.user?.role !== 'ADMIN') {
      return res.status(403).json({
